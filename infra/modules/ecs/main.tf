...
resource "aws_iam_role_policy" "ecs_task_kms" {
  name = "${var.project_name}-ecs-task-kms-${var.environment}"
  role = aws_iam_role.ecs_task.id
...
resource "aws_iam_role_policy" "ecs_task_textract" {
  name = "${var.project_name}-ecs-task-textract-${var.environment}"
  role = aws_iam_role.ecs_task.id
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "KMS_KEY_ID", value = var.kms_key_id },
